...
async function addUser(username, password, organization, email) {
    try{
        const hostID = crypto.randomUUID(username);
        const hash = await argon2.hash(password);
        const sql = `INSERT INTO Host (userid, username,password, organization, email)
                     VALUES (@userid, @username, @password, @organization, @email)`;
        const stmt = db.prepare(sql);
        stmt.run({userid, username, password, organization, email});
    } catch(e){
        console.error(e);
    }  
}
